function isEmpty(n){return!n||0===n.length||n==="0"}function deleteQuestion(n,t){$.ajax({url:"DeleteVotingQuestion",data:{id:n,qid:t},type:"POST",datatype:"json",success:function(){updateQuestions(n)}})}function editQuestion(n){$("#modalQuestionOk").off("click").on("click",function(){saveQuestion(n.votingId,n.index)});$("#newQuestionTitle").val(n.text);$("#newQuestionDescription").val(n.description);$("#questionType").val(n.type);n.type!=="1"&&($("#maxNoAnswers").val(n.maxNumAnswers),$("#maxNoAnswers").parent().show());$("#answersTable .answerOption").not(".hide").each(function(){$(this).detach()});$.each(n.answerOptions,function(n,t){const i=$("#answersTable").find("tr.hide").clone(!0).removeClass("hide");i.addClass("answer-line");i.children("td.answerOptionText").text(t.answer);i.children("td.answerOptionDescription").text(t.description);$("#answersTable").find("table").append(i)});$("#newQuestionModal").modal()}function saveQuestion(n,t){$("#modalQuestionOk").text("Speichere...");$("#modalQuestionOk").addClass("disabled");const f=$("#newQuestionDescription").val(),e=$("#newQuestionTitle").val(),o=parseInt($("#questionType").val()),r=[],u=[];$("#answersTable .answerOption").not(".hide").each(function(){r.push($(this).find("td.answerOptionText").text());u.push($(this).find("td.answerOptionDescription").text())});let i=parseInt($("#maxNoAnswers").val());isNaN(i)&&(i=1);$.post({url:"UpdateVotingQuestion",data:{id:n,qid:t,title:e,desc:f,type:o,answers:r,answerDescriptions:u,minNumAnswers:1,maxNumAnswers:i},success:function(){$("#newQuestionModal").modal("hide");resetNewQuestionModal();updateQuestions(n)}})}function formatDateTimeSeconds(n){var t=new Date(0);return t.setUTCSeconds(n),`${t.getDate()}.${(t.getMonth()+1).toString().padStart(2,"0")}.${t.getFullYear()} ${t.getHours()}:${t.getMinutes().toString().padStart(2,"0")}`}function updateQuestions(n){$("#questionList").load(`QuestionList?id=${n}`)}function resetNewQuestionModal(){$("#newQuestionTitle").val("");$("#newQuestionDescription").val("");$("#maxNoAnswers").val("1");$("#answersTable .answerOption").not(".hide").each(function(){$(this).detach()});$("#modalQuestionOk").text("OK");$("#modalQuestionOk").removeClass("disabled")}function grantRegistration(n,t){$.post({url:"../Registration/GrantRegistration",data:{rids:n},success:function(){updateRegistrations(t)}})}function denyRegistration(n,t){$.post({url:"../Registration/DenyRegistration",data:{rid:n},success:function(){updateRegistrations(t)}})}function showCompletedRegistrations(n){const i=$("#grantedRegistrationsList");i.empty();const r=$("#deniedRegistrationsList");r.empty();let u=0,f=0;for(var t=0;t<n.length;t++){const e=n[t],o=`<div class="m-0 px-3 border-bottom">${e.voterName}</div>`;e.decision===1?(i.append(o),u++):(r.append(o),f++)}$("#grantedRegistrationsBadge").text(u);$("#deniedRegistrationsBadge").text(f)}function updateRegistrations(n){$.ajax({url:"../Registration/GetRegistrations",data:{votingId:n},type:"POST",datatype:"json",success:function(t){showRegistrations(t,n)}});$.ajax({url:"../Registration/GetCompletedRegistrations",data:{votingId:n},type:"POST",datatype:"json",success:function(n){showCompletedRegistrations(n)}})}function showRegistrations(n,t){$("#openRegistrationsList").removeClass("openRegistrationItem");for(let i=0;i<n.length;i++){const r=n[i],u=`<div class="d-flex mx-3 my-0 border-bottom openRegistrationItem" id="openreg-${r.registrationId}"><div style="flex:1;margin-right:1rem">${r.voterName}</div>
